...
net start cntlm
```
В случае успешного запуска сервиса в консоли появится сообщение следующего вида:
```
Служба "Cntlm Authentication Proxy" успешно запущена.
```
При запуске возможно возникновение ошибки слеующего вида:
```
Не удалось запустить службу "Cntlm Authentication Proxy".
Системная ошибка.
Системная ошибка 1067.
Процесс был неожиданно завершен.
```
В этом случае нужно попробовать выполнить команду запуска ещё раз.
## Настройка *Npm*
