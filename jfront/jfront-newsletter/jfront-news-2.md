...
О возможности развития продукта в долгосрочном периоде нужно заботиться с самого начала («Спасибо, Кэп!» ;).

Если такой опыт есть, понятно, что его нужно применять («Спасибо, Кэп!» ;).
...
новыми терминами и понятиями (хотя в какой-то степени и это тоже присутствует), сколько в том, что часть этих терминов в
её контексте приобретает некоторый дополнительный, а иногда и особенный смысл.  
...
- *Entities* - "бизнес-правила, независимые от приложения" (а не "Сущности" - не то, что вы подумали))
...
На этом выпуск завершён, а следующий предполагается посвятить подробному рассказу о *Dependency Inversion* - главном
паттерне *правила зависимостей*, а значит и главном паттерне *Чистой Архитектуры*.

## Главный паттерн *Чистой Архитектуры*

TBD

Дальнейшее изложение будет посвящено раскрытию смысла этого важнейшего паттерна. 

### В чём проблема ?

Когда UI-код вызывает код бизнес-логики, то 
Проблема в том, что 
Идея *Dependency Inversion*
Не бизнес-логика вызывает 

### Презентеры в MVP
Хорошо известный нам пример применения *Dependency Inversion*
...
<p align="center">
  <img src="images/ca-main-dependency-rule-3.svg" alt="drawing"/>
</p>
...
Из-за того, что классическая диаграмма показывает лишь частные случаи применения подхода DI, у читателей может
возникать неполное понимание самой идеи. 
 
Диаграмма более общего вида снимает эти частности:

<p align="center">
  <img src="images/ca-plugin-tree.svg" alt="drawing"/>
</p>

Подход, при котором развитие

Поддерживает сразу два принципа SOLID:
- Open-Closed
- Dependency-Inversion

Об успешности подобного подхода свидетельствуют истории успеха таких широко известных продуктов, как *Spring*,
*Eclipse*, *IDEA*, *Maven*, *Chrome* *VS Code* и многих других.

Продукты, имеющие плагинную архитектуру, отличает наличие общих правил создания новых плагинов и их подключения к
семейству других, уже существующих плагинов продукта. Как правило, при этом предполагается определённая динамичность
сборки. 

## Подробно Dependency Inversion

IoC, DI - Dependency Inversion (не путать с другим DI - Dependency Injection)
Adapter

## Тестирование
Избегать хрупких тестов - тот же подход, преследующий *устойчивость к изменениям*

### *react-testing-library*
Стимулирует написание тестов, устойчивых к изменениям кода
